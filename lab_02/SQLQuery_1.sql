...
WHERE P1.ProductMaterial <> P2.ProductMaterial AND P1.ProductMadeIn = 'China'
...
FROM (tblCompany JOIN tblProduct ON tblCompany.CompanyID = tblProduct.CompanyID)
JOIN tblBill ON tblBill.BillProductID = tblProduct.ProductID
WHERE tblCompany.CompanyName LIKE '%Inc' AND tblCompany.CompanyID IN
(
...
SELECT ProductName, ProductMaterial
FROM tblProduct
WHERE EXISTS
(
...
-- List of products, which are made in China and its price is bigger than products made in Germany
/*
Select ProductMadeIn, BillPrice
FROM dbo.tblProduct INNER JOIN tblBill ON tblProduct.ProductID = tblBill.BillProductID 
WHERE (ProductMadeIn LIKE 'Germany%');
*/
...
(
	SELECT BillPrice
	FROM tblBill JOIN tblProduct ON ProductID = BillProductID
	WHERE ProductMadeIn LIKE 'Germany%' -- MAX: 9583257
)
AND ProductMadeIn LIKE 'China%'
...
IF OBJECT_ID('tempdb..#HighestPrice') IS NOT NULL
	DROP TABLE #HighestPrice
ELSE
SELECT SellerID,
	   SellerFirstName,
	   SellerLastName,
	   ...
	   MAX(BillPrice) AS 'Maximum Price'
INTO #HighestPrice
FROM tblSeller INNER JOIN tblBill ON BillSellerID = SellerID
	GROUP BY SellerID, SellerFirstName, SellerLastName, SellerGender, SellerVisaCardSeries
...
-- Top 10 Buyer having higest ordered times 
SELECT TOP 10
	   BillBuyerID AS 'ReceiverID',
	   COUNT(BillBuyerID) AS 'Times',
	   SUM(BillQuantity) AS 'Total Quantity',
	   '$' + CAST(SUM(BillPrice) AS VARCHAR(12)) AS 'Total Price'
FROM tblBill
--WHERE BillPrice > 100000
GROUP BY BillBuyerID
ORDER BY Times DESC
...
FROM tblBill
GROUP BY BillSellerID, BillProductID
HAVING BillSellerID = 7

--Request #16: INSERT
INSERT INTO tblBill(BillSellerID, BillBuyerID, BillProductID, BillQuantity, BillPrice)
VALUES (1, 999, 500, 12800, 1024000)

-- Request #17: INSERT + Select
INSERT tblBill(BillSellerID, BillBuyerID, BillProductID, BillQuantity, BillPrice)
SELECT(
			SELECT TOP 1 BillSellerID
	  ...
	  (		
			...
			FROM tblBuyer
			WHERE BuyerLastName LIKE '%er'
			ORDER BY BuyerFirstName DESC
	  ),
	  5, 1000000, 9750000
...
DELETE tblBill
WHERE BillBuyerID = 100 AND BillProductID = 100 AND BillSellerID = 100
...
-- Define OTB
--SELECT * FROM dbo.TestBook
WITH Details(BookID, BookAuthorID, BookTitle, BookEdition, BookPrice, Deal)
AS
(
...
FROM
(
	SELECT BillSellerID, BillProductID, BillBuyerID
	FROM tblBill
) AS SourceTable
PIVOT
(
...
-- unpivot table
SELECT BillSellerID, BillProductID, BillCountBuyer
FROM
(
